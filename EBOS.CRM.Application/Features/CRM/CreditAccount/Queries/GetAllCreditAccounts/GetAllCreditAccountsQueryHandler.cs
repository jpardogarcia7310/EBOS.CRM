...
public class GetAllCreditAccountsQueryHandler(ICreditAccountRepository repository, IMapper mapper)
    : IRequestHandler<GetAllCreditAccountsQuery, IReadOnlyCollection<CreditAccountResponse>>
{
    ...
    public async Task<IReadOnlyCollection<CreditAccountResponse>> Handle(GetAllCreditAccountsQuery request, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();

        var entities = await _repository.GetAllAsync(cancellationToken);
        return _mapper.Map<IReadOnlyCollection<CreditAccountResponse>>(entities);
    }
}
