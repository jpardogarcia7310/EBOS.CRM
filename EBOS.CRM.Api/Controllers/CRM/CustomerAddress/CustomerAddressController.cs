...
public class CustomerAddressController(IMediator mediator, IStringLocalizer<SharedResource> localizer) : ControllerBase
{
    #region Commands
    [HttpPost]
    [Produces("application/json")]
        ...
        [FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 50, CancellationToken cancellationToken = default)
    {
        ...
        var result = await mediator.Send(new GetAllCustomerAddressesQuery(safePageNumber, safePageSize),
            cancellationToken);
