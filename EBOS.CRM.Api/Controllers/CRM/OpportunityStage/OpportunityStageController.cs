...
namespace EBOS.CRM.Api.Controllers.CRM.OpportunityStage;

[ApiController]
[ApiVersion("2.0")]
[Route(ApiRouteTemplates.Versioned)]
[Produces("application/json")]
public class OpportunityStageController(IMediator mediator) : ControllerBase
{
    #region Commands
    [HttpPost]
    [Produces("application/json")]
    [ProducesResponseType(typeof(OpportunityStageResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> AddAsync([FromBody] AddOpportunityStageRequest request,
        CancellationToken cancellationToken = default)
    {
        return Ok(await mediator.Send(new AddOpportunityStageCommand(request), cancellationToken));
    }
    ...
    public async Task<IActionResult> UpdateAsync([FromRoute] long id, [FromBody] UpdateOpportunityStageRequest request,
        CancellationToken cancellationToken = default)
    {
        var dto = await mediator.Send(new UpdateOpportunityStageCommand(id, request), cancellationToken);
        if (dto is null)
        {
            return NotFound(ProblemDetailsFactory.CreateProblemDetails(HttpContext,
                ...
                detail: $"OpportunityStage with id {id} not found."));
        }
    ...
    {
        var dto = await mediator.Send(new GetOpportunityStageByIdQuery(id), cancellationToken);
        if (dto is null)
        {
            return NotFound(ProblemDetailsFactory.CreateProblemDetails(HttpContext,
                statusCode: StatusCodes.Status404NotFound, title: ProblemDetailsDefaults.NotFoundTitle,
    ...
    [HttpGet]
    [ProducesResponseType(typeof(IReadOnlyCollection<OpportunityStageResponse>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await mediator.Send(new GetAllOpportunityStagesQuery(safePageNumber, safePageSize), cancellationToken);
