using System.Net;
using System.Net.Http.Json;
using EBOS.CRM.Api.IntegrationTests.Infrastructure;
using EBOS.CRM.Api.IntegrationTests.TestUtils;
using EBOS.CRM.Application.Contracts.Requests.CRM.CorporateCustomer;
using EBOS.CRM.Domain.Interfaces.Repositories.CRM;
using FluentAssertions;
using Microsoft.AspNetCore.Hosting;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Extensions.DependencyInjection;

namespace EBOS.CRM.Api.IntegrationTests.Controllers.CRM.CorporateCustomer;

public class CorporateCustomerErrorHandlingTest(
    CorporateCustomerErrorHandlingTest.FailingCorporateCustomerFactory factory)
    : IClassFixture<CorporateCustomerErrorHandlingTest.FailingCorporateCustomerFactory>
{
    private readonly HttpClient _client = factory.CreateClient();
    private readonly string _version = ApiVersionHelper.GetLatestVersion(factory, "CorporateCustomer");

    [Fact]
    public async Task GetAll_Returns_500_WhenRepositoryFails()
    {
        var response = await _client.GetAsync($"/api/v{_version}/CorporateCustomer");
        response.StatusCode.Should().Be(HttpStatusCode.InternalServerError);

        var problem = await response.Content.ReadFromJsonAsync<ProblemDetails>();
        problem.Should().NotBeNull();
        problem!.Status.Should().Be(500);
    }

    [Fact]
    public async Task Add_Returns_500_WhenRepositoryFails()
    {
        var request = new AddCorporateCustomerRequest(
            TenantId: 1,
            Code: "CORP-001",
            Email: "corp@example.com",
            Phone: "+1 555 0000",
            StatusId: 1,
            LegalName: "Contoso S.A.",
            TaxIdentification: "A12345678");

        var response = await _client.PostAsJsonAsync($"/api/v{_version}/CorporateCustomer", request);
        response.StatusCode.Should().Be(HttpStatusCode.InternalServerError);
    }

    [Fact]
    public async Task Update_Returns_500_WhenRepositoryFails()
    {
        var request = new UpdateCorporateCustomerRequest(
            TenantId: 1,
            Code: "CORP-001",
            Email: "corp@example.com",
            Phone: "+1 555 0000",
            StatusId: 1,
            LegalName: "Contoso S.A.",
            TaxIdentification: "A12345678");

        var response = await _client.PutAsJsonAsync($"/api/v{_version}/CorporateCustomer/1", request);
        response.StatusCode.Should().Be(HttpStatusCode.InternalServerError);
    }

    [Fact]
    public async Task Delete_Returns_500_WhenRepositoryFails()
    {
        var response = await _client.DeleteAsync($"/api/v{_version}/CorporateCustomer/1");
        response.StatusCode.Should().Be(HttpStatusCode.InternalServerError);
    }

    public sealed class FailingCorporateCustomerFactory : CustomWebApplicationFactory
    {
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            base.ConfigureWebHost(builder);
            builder.ConfigureServices(services =>
            {
                var descriptor = services.SingleOrDefault(
                    d => d.ServiceType == typeof(ICorporateCustomerRepository));
                if (descriptor != null)
                {
                    services.Remove(descriptor);
                }

                services.AddScoped<ICorporateCustomerRepository, FailingCorporateCustomerRepository>();
            });
        }
    }
}



