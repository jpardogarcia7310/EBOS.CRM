    ...
    IClassFixture<CustomWebApplicationFactory<Program>> // Your API's Program.cs file
{
    ...
    {
        var response = await _client.GetAsync($"/api/{_version}/CorporateCustomer");
    ...
    public async Task GetCorporateCustomerById_ExistingId_ReturnsCorporateCustomer()
    {
        var id = await ControllerTestHelper.GetFirstIdAsync<CorporateCustomerResponse>(
            _client, $"/api/{_version}/CorporateCustomer", x => x.Id);

        var response = await _client.GetAsync($"/api/{_version}/CorporateCustomer/{id}");
        response.EnsureSuccessStatusCode();
    ...
    public async Task GetCorporateCustomerById_NonExistingId_ReturnsNotFound()
    {
        ...
        var response = await _client.GetAsync($"/api/{_version}/CorporateCustomer/{id + 9999}");
    ...
    {
        var response = await _client.GetAsync($"/api/{_version}/CorporateCustomer/simulate-db-failure");
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    {
        var response = await _client.GetAsync($"/api/{_version}/CorporateCustomer/simulate-timeout");
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    {
        var response1 = await _client.GetAsync($"/api/{_version}/CorporateCustomer/simulate-db-failure");
        Assert.Equal(HttpStatusCode.NotFound, response1.StatusCode);

        var response2 = await _client.GetAsync($"/api/{_version}/CorporateCustomer");
    ...
    {
        var response1 = await _client.GetAsync($"/api/{_version}/CorporateCustomer/simulate-timeout");
        Assert.Equal(HttpStatusCode.NotFound, response1.StatusCode);
