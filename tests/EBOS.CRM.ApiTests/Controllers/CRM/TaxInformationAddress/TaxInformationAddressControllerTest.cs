    ...
    IClassFixture<CustomWebApplicationFactory<Program>> // Your API's Program.cs file
{
    ...
    {
        var response = await _client.GetAsync($"/api/{_version}/TaxInformationAddress");
    ...
    public async Task GetTaxInformationAddressById_ExistingId_ReturnsTaxInformationAddress()
    {
        var id = await ControllerTestHelper.GetFirstIdAsync<TaxInformationAddressResponse>(
            _client, $"/api/{_version}/TaxInformationAddress", x => x.Id);

        var response = await _client.GetAsync($"/api/{_version}/TaxInformationAddress/{id}");
        response.EnsureSuccessStatusCode();
    ...
    public async Task GetTaxInformationAddressById_NonExistingId_ReturnsNotFound()
    {
        ...
        var response = await _client.GetAsync($"/api/{_version}/TaxInformationAddress/{id + 9999}");
    ...
    {
        var response = await _client.GetAsync($"/api/{_version}/TaxInformationAddress/simulate-db-failure");
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    {
        var response = await _client.GetAsync($"/api/{_version}/TaxInformationAddress/simulate-timeout");
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    {
        var response1 = await _client.GetAsync($"/api/{_version}/TaxInformationAddress/simulate-db-failure");
        Assert.Equal(HttpStatusCode.NotFound, response1.StatusCode);

        var response2 = await _client.GetAsync($"/api/{_version}/TaxInformationAddress");
    ...
    {
        var response1 = await _client.GetAsync($"/api/{_version}/TaxInformationAddress/simulate-timeout");
        Assert.Equal(HttpStatusCode.NotFound, response1.StatusCode);
