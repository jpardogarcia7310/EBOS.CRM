    ...
    IClassFixture<CustomWebApplicationFactory<Program>> // Your API's Program.cs file
{
    ...
    public async Task GetCustomerById_ExistingId_ReturnsCustomer()
    {
        var id = await ControllerTestHelper.GetFirstIdAsync<CustomerResponse>(
            _client, $"/api/{_version}/Customer", x => x.Id);

        var response = await _client.GetAsync($"/api/{_version}/Customer/{id}");
        response.EnsureSuccessStatusCode();
    ...
    public async Task GetCustomerById_NonExistingId_ReturnsNotFound()
    {
        ...
        var response = await _client.GetAsync($"/api/{_version}/Customer/{id + 9999}");
    ...
    {
        var response = await _client.GetAsync($"/api/{_version}/Customer/simulate-db-failure");
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    {
        var response = await _client.GetAsync($"/api/{_version}/Customer/simulate-timeout");
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    {
        var response1 = await _client.GetAsync($"/api/{_version}/Customer/simulate-db-failure");
        Assert.Equal(HttpStatusCode.NotFound, response1.StatusCode);

        var response2 = await _client.GetAsync($"/api/{_version}/Customer");
    ...
    {
        var response1 = await _client.GetAsync($"/api/{_version}/Customer/simulate-timeout");
        Assert.Equal(HttpStatusCode.NotFound, response1.StatusCode);
