...
public class EstadosControllerTest(WebApplicationFactory<Program> factory) : IClassFixture<WebApplicationFactory<Program>> // Program.cs de tu API
{
    ...
    public async Task GetAllStatuses_ReturnsSuccessAndList()
    {
        ...
        var statuses = await response.Content.ReadFromJsonAsync<IEnumerable<EstadoResponseDto>>();
