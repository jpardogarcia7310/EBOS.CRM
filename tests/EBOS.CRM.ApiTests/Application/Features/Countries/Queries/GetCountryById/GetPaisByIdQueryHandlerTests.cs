...
public class GetPaisByIdQueryHandlerTests
{
    private readonly Mock<IPaisRepository> _repositoryMock;
    private readonly Mock<IMapper> _mapperMock;
    private readonly GetPaisByIdQueryHandler _handler;

    public GetPaisByIdQueryHandlerTests()
    {
        _repositoryMock = new Mock<IPaisRepository>();
        _mapperMock = new Mock<IMapper>();
        _handler = new GetPaisByIdQueryHandler(_repositoryMock.Object, _mapperMock.Object);
    }
    ...
    {
        // Arrange
        var country = new Pais { Id = 1, Name = "España", Iso31661A2Code = "ES", Iso31661A3Code = "ESP", Iso31661NumCode = "724", Domain = ".es", Currency = "Euro", CurrencyCode = "EUR", InternationalPhoneCode = "34" };
        var dto = new PaisResponseDto(country.Id, country.Name, country.Iso31661A2Code, country.Iso31661A3Code, country.Iso31661NumCode, country.Domain, country.Currency, country.CurrencyCode, country.InternationalPhoneCode);

        _repositoryMock.Setup(r => r.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync(country);
        _mapperMock.Setup(m => m.Map<PaisResponseDto>(country)).Returns(dto);
        ...
        _mapperMock.Verify(m => m.Map<PaisResponseDto>(country), Times.Once);
    }
    ...
    {
        // Arrange
        _repositoryMock.Setup(r => r.GetByIdAsync(99, It.IsAny<CancellationToken>())).ReturnsAsync((Pais?)null);
        var query = new GetPaisByIdQuery(99);
        ...
        // Assert
        Assert.Null(result);
        _mapperMock.Verify(m => m.Map<PaisResponseDto>(It.IsAny<Pais>()), Times.Never);
    }
    ...
    {
        // Arrange
        var country = new Pais { Id = 1, Name = "España" };
        _repositoryMock.Setup(r => r.GetByIdAsync(1, It.IsAny<CancellationToken>()))
                       .ReturnsAsync(country);

        // Simulamos que el mapper de Mapster falla
        _mapperMock.Setup(m => m.Map<PaisResponseDto>(country))
    ...
    {
        // Arrange
        var query = new GetPaisByIdQuery(42);
        _repositoryMock.Setup(r => r.GetByIdAsync(42, It.IsAny<CancellationToken>())).ReturnsAsync((Pais?)null);
    ...
    {
        // Arrange
        _repositoryMock.Setup(r => r.GetByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync((Pais?)null);
        var query = new GetPaisByIdQuery(1);
