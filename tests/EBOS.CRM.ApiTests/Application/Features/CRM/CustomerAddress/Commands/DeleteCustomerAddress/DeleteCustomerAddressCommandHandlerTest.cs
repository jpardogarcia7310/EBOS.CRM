...
public class DeleteCustomerAddressCommandHandlerTest
{
    ...
    public DeleteCustomerAddressCommandHandlerTest()
    {
    ...
    public async Task Handle_WhenEntityNotFound_ReturnsFalse()
    {
        ...
        var result = await _handler.Handle(new DeleteCustomerAddressCommand(1), CancellationToken.None);

        Assert.False(result);
        _repositoryMock.Verify(r => r.DeleteAsync(It.IsAny<CRMCustomerAddress>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task Handle_WhenSaveChangesThrows_RollsBack()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _handler.Handle(new DeleteCustomerAddressCommand(1), CancellationToken.None));
