...
public class UpdateCustomerAddressCommandHandlerTest
{
    ...
    public UpdateCustomerAddressCommandHandlerTest()
    {
    ...
    public async Task Handle_WhenEntityNotFound_ReturnsNull()
    {
        _repositoryMock.Setup(r => r.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync((CRMCustomerAddress?)null!);

        var result = await _handler.Handle(new UpdateCustomerAddressCommand(1, BuildUpdateRequest()), CancellationToken.None);

        Assert.Null(result);
        _repositoryMock.Verify(r => r.UpdateAsync(It.IsAny<CRMCustomerAddress>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    {
        var entity = new CRMCustomerAddress();

        _repositoryMock.Setup(r => r.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
        _mapperMock.Setup(m => m.Map(It.IsAny<UpdateCustomerAddressRequest>(), entity))
            .Returns(entity);
        _mapperMock.Setup(m => m.Map<CustomerAddressResponse>(entity))
            .Returns(TestResponseFactory.Create<CustomerAddressResponse>());
    ...
    public async Task Handle_WhenSaveChangesThrows_RollsBack()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _handler.Handle(new UpdateCustomerAddressCommand(1, BuildUpdateRequest()), CancellationToken.None));
