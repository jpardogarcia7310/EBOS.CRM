...
public class DeleteBranchOfficeCommandHandlerTest
{
    private readonly Mock<IBranchOfficeRepository> _repositoryMock;
    private readonly Mock<IAuditService> _auditServiceMock;
    private readonly DeleteBranchOfficeCommandHandler _handler;

    public DeleteBranchOfficeCommandHandlerTest()
    {
        ...
        _handler = new DeleteBranchOfficeCommandHandler(
    ...
    public async Task Handle_WhenEntityNotFound_ReturnsFalse()
    {
        ...
        var result = await _handler.Handle(new DeleteBranchOfficeCommand(1), CancellationToken.None);

        Assert.False(result);
        _repositoryMock.Verify(r => r.DeleteAsync(It.IsAny<CRMBranchOffice>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task Handle_WhenSaveChangesThrows_RollsBack()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _handler.Handle(new DeleteBranchOfficeCommand(1), CancellationToken.None));
