...
public class UpdateTaxInformationCommandHandlerTest
{
    private readonly Mock<ITaxInformationRepository> _repositoryMock;
    private readonly Mock<IAuditService> _auditServiceMock;
    private readonly Mock<IMapper> _mapperMock;
    private readonly UpdateTaxInformationCommandHandler _handler;

    public UpdateTaxInformationCommandHandlerTest()
    {
        ...
        _handler = new UpdateTaxInformationCommandHandler(
    ...
    public async Task Handle_WhenEntityNotFound_ReturnsNull()
    {
        ...
        var result = await _handler.Handle(new UpdateTaxInformationCommand(1, BuildUpdateRequest()), CancellationToken.None);

        Assert.Null(result);
        _repositoryMock.Verify(r => r.UpdateAsync(It.IsAny<CRMTaxInformation>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task Handle_WhenEntityFound_UpdatesAndAudits()
    {
        ...
        _repositoryMock.Setup(r => r.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
        _mapperMock.Setup(m => m.Map(It.IsAny<UpdateTaxInformationRequest>(), entity))
            .Returns(entity);
        _mapperMock.Setup(m => m.Map<TaxInformationResponse>(entity))
            .Returns(TestResponseFactory.Create<TaxInformationResponse>());
    ...
    public async Task Handle_WhenSaveChangesThrows_RollsBack()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _handler.Handle(new UpdateTaxInformationCommand(1, BuildUpdateRequest()), CancellationToken.None));
