...
public class AddTaxInformationCommandHandlerTest
{
    ...
    public AddTaxInformationCommandHandlerTest()
    {
    ...
    public async Task Handle_WhenRequestIsValid_AddsAndAudits()
    {
        ...
        _mapperMock.Setup(m => m.Map<CRMTaxInformation>(request)).Returns(entity);
        _mapperMock.Setup(m => m.Map<TaxInformationResponse>(entity)).Returns(TestResponseFactory.Create<TaxInformationResponse>());

        var result = await _handler.Handle(new AddTaxInformationCommand(request), CancellationToken.None);
    ...
    public async Task Handle_WhenSaveChangesThrows_RollsBack()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _handler.Handle(new AddTaxInformationCommand(request), CancellationToken.None));
