...
public class DeleteCustomerCommandHandlerTest
{
    ...
    public async Task Handle_WhenEntityNotFound_ReturnsFalse()
    {
        ...
        Assert.False(result);
        _repositoryMock.Verify(r => r.DeleteAsync(It.IsAny<CRMCustomer>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task Handle_WhenSaveChangesThrows_RollsBack()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _handler.Handle(new DeleteCustomerCommand(1), CancellationToken.None));
