...
public class UpdateCreditAccountCommandHandlerTest
{
    private readonly Mock<ICreditAccountRepository> _repositoryMock;
    private readonly Mock<IAuditService> _auditServiceMock;
    private readonly Mock<IMapper> _mapperMock;
    private readonly UpdateCreditAccountCommandHandler _handler;

    public UpdateCreditAccountCommandHandlerTest()
    {
        ...
        _handler = new UpdateCreditAccountCommandHandler(
    ...
    public async Task Handle_WhenEntityNotFound_ReturnsNull()
    {
        ...
        var result = await _handler.Handle(new UpdateCreditAccountCommand(1, BuildUpdateRequest()), CancellationToken.None);

        Assert.Null(result);
        _repositoryMock.Verify(r => r.UpdateAsync(It.IsAny<CRMCreditAccount>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task Handle_WhenEntityFound_UpdatesAndAudits()
    {
        ...
        _repositoryMock.Setup(r => r.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(entity);
        _mapperMock.Setup(m => m.Map(It.IsAny<UpdateCreditAccountRequest>(), entity))
            .Returns(entity);
        _mapperMock.Setup(m => m.Map<CreditAccountResponse>(entity))
            .Returns(TestResponseFactory.Create<CreditAccountResponse>());
    ...
    public async Task Handle_WhenSaveChangesThrows_RollsBack()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _handler.Handle(new UpdateCreditAccountCommand(1, BuildUpdateRequest()), CancellationToken.None));
