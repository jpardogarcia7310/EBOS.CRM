...
public class DeleteBranchOfficeAddressCommandHandlerTest
{
    private readonly Mock<IBranchOfficeAddressRepository> _repositoryMock;
    private readonly Mock<IAuditService> _auditServiceMock;
    private readonly DeleteBranchOfficeAddressCommandHandler _handler;

    public DeleteBranchOfficeAddressCommandHandlerTest()
    {
        ...
        _handler = new DeleteBranchOfficeAddressCommandHandler(
    ...
    public async Task Handle_WhenEntityNotFound_ReturnsFalse()
    {
        ...
        var result = await _handler.Handle(new DeleteBranchOfficeAddressCommand(1), CancellationToken.None);

        Assert.False(result);
        _repositoryMock.Verify(r => r.DeleteAsync(It.IsAny<CRMBranchOfficeAddress>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task Handle_WhenSaveChangesThrows_RollsBack()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _handler.Handle(new DeleteBranchOfficeAddressCommand(1), CancellationToken.None));
