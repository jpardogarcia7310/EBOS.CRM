...
public class AddIndividualCustomerCommandHandlerTest
{
    ...
    public AddIndividualCustomerCommandHandlerTest()
    {
    ...
    public async Task Handle_WhenRequestIsValid_AddsAndAudits()
    {
        ...
        _mapperMock.Setup(m => m.Map<CRMIndividualCustomer>(request)).Returns(entity);
        _mapperMock.Setup(m => m.Map<IndividualCustomerResponse>(entity)).Returns(TestResponseFactory.Create<IndividualCustomerResponse>());

        var result = await _handler.Handle(new AddIndividualCustomerCommand(request), CancellationToken.None);
    ...
    public async Task Handle_WhenSaveChangesThrows_RollsBack()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _handler.Handle(new AddIndividualCustomerCommand(request), CancellationToken.None));
