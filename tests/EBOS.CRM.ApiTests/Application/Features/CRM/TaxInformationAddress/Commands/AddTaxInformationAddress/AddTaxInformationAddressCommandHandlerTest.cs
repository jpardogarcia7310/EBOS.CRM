...
public class AddTaxInformationAddressCommandHandlerTest
{
    ...
    private readonly AddTaxInformationAddressCommandHandler _handler;

    public AddTaxInformationAddressCommandHandlerTest()
    {
        ...
        _handler = new AddTaxInformationAddressCommandHandler(
    ...
    public async Task Handle_WhenRequestIsValid_AddsAndAudits()
    {
        ...
        _mapperMock.Setup(m => m.Map<CRMTaxInformationAddress>(request)).Returns(entity);
        _mapperMock.Setup(m => m.Map<TaxInformationAddressResponse>(entity)).Returns(TestResponseFactory.Create<TaxInformationAddressResponse>());

        var result = await _handler.Handle(new AddTaxInformationAddressCommand(request), CancellationToken.None);
    ...
    public async Task Handle_WhenSaveChangesThrows_RollsBack()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _handler.Handle(new AddTaxInformationAddressCommand(request), CancellationToken.None));
