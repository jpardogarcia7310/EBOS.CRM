...
public class DeleteTaxInformationAddressCommandHandlerTest
{
    private readonly Mock<ITaxInformationAddressRepository> _repositoryMock;
    private readonly Mock<IAuditService> _auditServiceMock;
    private readonly DeleteTaxInformationAddressCommandHandler _handler;

    public DeleteTaxInformationAddressCommandHandlerTest()
    {
        ...
        _handler = new DeleteTaxInformationAddressCommandHandler(
    ...
    public async Task Handle_WhenEntityNotFound_ReturnsFalse()
    {
        ...
        var result = await _handler.Handle(new DeleteTaxInformationAddressCommand(1), CancellationToken.None);

        Assert.False(result);
        _repositoryMock.Verify(r => r.DeleteAsync(It.IsAny<CRMTaxInformationAddress>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task Handle_WhenSaveChangesThrows_RollsBack()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _handler.Handle(new DeleteTaxInformationAddressCommand(1), CancellationToken.None));
