...
namespace EBOS.CRM.ApiTests.Application.Features.EBOS.Countries.Queries.GetAllCountries;
...
public class GetAllCountriesQueryHandlerTest
{
    ...
    public async Task Handle_NoCountries_ReturnsEmptyEnumerable()
    {
        ...
        _repositoryMock.Setup(r => r.CountAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync(0);
        _mapperMock.Setup(m => m.Map<IReadOnlyCollection<CountryResponse>>(countries)).Returns(new List<CountryResponse>());
