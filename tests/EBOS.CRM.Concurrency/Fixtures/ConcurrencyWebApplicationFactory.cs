...
namespace EBOS.CRM.Concurrency.Fixtures;

public sealed class ConcurrencyWebApplicationFactory<TProgram> : WebApplicationFactory<TProgram>
    where TProgram : class
{
    private readonly string _connectionString = $"DataSource=ConcurrencyTests_{Guid.NewGuid():N};Mode=Memory;Cache=Shared";
    ...
    private static void ConfigureConnection(SqliteConnection connection)
    {
        ...
        connection.CreateFunction<DateTime>("SYSUTCDATETIME", () => DateTime.UtcNow);
        connection.CreateFunction<DateTime>("GETUTCDATE", () => DateTime.UtcNow);
